...
export type AppState = { householdName: string; members: Member[]; incomeHistory: IncomeEntry[]; budget: Budget; archivedBudgets: Budget[]; surplusCents: number; projects: Project[]; surplusEntries: SurplusEntry[] };
...
export function resetState(): AppState {
  ...
  clean.budget.payments = [];
  clean.budget.status = "ACTIVE";
  delete clean.budget.closedSurplusCents;
  clean.surplusCents = 0;
  clean.surplusEntries = [];
  clean.projects = [];
  clean.archivedBudgets = [];
  return clean;
}
